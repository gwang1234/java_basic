...
> **다형적 참조, 메서드 오버라이딩을 활용해 다른 클래스들을 한 타입에 담아 코드 중복을 줄일 수 있다**

<br>

> 좋은 코드는 새로운 기능을 추가해도 변경범위가 최소화 되었을때 좋은 코드라고 할 수 있다

<br>

**but,**  
이런 다형성에도 문제가 있다
1. 실수로 상위 클래스로 객체를 생성했을시 제대로 된 기능을 수행할 수 없다
2. 자식 클래스가 메서드 오버라이딩을 하지 않았을 시 제대로 된 기능을 수행할 수 없다

<br>

_추상 클래스와 추상 메서드로 해결할 수 있다_
...
- 순수 추상 클래스는 코드를 실행할 바디 부분이 없다
- 자바는 순수 추상 클래스르 더 편리하게 사용할 수 있도록 **인터페이스**를 제공

### 인터페이스
- 인스턴스를 생성할 수 없다
- 상속시 모든 메서드를 오버라이딩 해야 한다
- 주로 다형성을 위해 사용된다
- 인터페이스 메서드는 모두 `public`, `abstact`로 생략할 수 있다
- 다중 상속 지원
...
**상속**: 부모의 기능을 자식이 상속 받을때, 상속이라고 한다  
**구현**: 부모 인터페이스의 기능을 자식이 상속 받을 때는 인터페이스 구현한다고 한다

### 인터페이스 - 다중구현
- 인터페이스는 다중구현을 허용한다
- 인터페이스는 모두 추상 메서드로 이루어져 있기 때문
- 메모리상에서 실질적인 기능은 자식이 해주니까 다중구현 가능 -> 다이아몬드 문제 발생 x

### OCP(Open-Closed Priciple) 원칙
- Open for extension: 새로운 기능의 추가나 변경사항이 생겼을 떄, 기존 코드는 확장할 수 있어야 한다
- Closed for modification: 기존의 코드는 수정되지 말아야 한다
